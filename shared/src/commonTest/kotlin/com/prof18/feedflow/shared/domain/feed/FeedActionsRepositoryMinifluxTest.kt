...
class FeedActionsRepositoryMinifluxTest : KoinTestBase() {
    ...
    fun `markAsRead should call gReaderRepository and update state`() = runTest(testDispatcher) {
        ...
        val feedItems = listOf(
            buildFeedItem("00000000000008cd", "Article 1", 10000L, feedSource),
            buildFeedItem("000000000000081f", "Article 2", 9000L, feedSource),
            buildFeedItem("00000000000004ae", "Article 3", 8000L, feedSource),
        )
    ...
    fun `markAllAboveAsRead should mark items above target as read`() = runTest(testDispatcher) {
        ...
        val feedItems = listOf(
            ...
            buildFeedItem("000000000000081f", "Article 2", 9000L, feedSource),
            buildFeedItem("00000000000004ae", "Article 3", 8000L, feedSource),
            buildFeedItem("0000000000000834", "Article 4", 7000L, feedSource),
            buildFeedItem("000000000000090b", "Article 5", 6000L, feedSource),
        )
        databaseHelper.insertFeedItems(feedItems, lastSyncTimestamp = 0)
        feedStateRepository.getFeeds()
        advanceUntilIdle()
