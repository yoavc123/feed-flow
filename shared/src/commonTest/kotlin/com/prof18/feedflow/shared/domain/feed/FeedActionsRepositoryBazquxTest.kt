...
class FeedActionsRepositoryBazquxTest : KoinTestBase() {
    ...
    fun `markAsRead should call gReaderRepository and update state`() = runTest(testDispatcher) {
        ...
        val feedItems = listOf(
            buildFeedItem("000000030000aeb4", "Article 1", 10000L, feedSource),
            buildFeedItem("0000000400000de5", "Article 2", 9000L, feedSource),
            buildFeedItem("0000001c00000193", "Article 3", 8000L, feedSource),
        )
    ...
    fun `markAllAboveAsRead should mark items above target as read`() = runTest(testDispatcher) {
        ...
        val feedItems = listOf(
            ...
            buildFeedItem("0000000400000de5", "Article 2", 9000L, feedSource),
            buildFeedItem("0000001c00000193", "Article 3", 8000L, feedSource),
            buildFeedItem("0000000d00070785", "Article 4", 7000L, feedSource),
            buildFeedItem("000000030000aeb5", "Article 5", 6000L, feedSource),
        )
        databaseHelper.insertFeedItems(feedItems, lastSyncTimestamp = 0)
        feedStateRepository.getFeeds()
        advanceUntilIdle()
